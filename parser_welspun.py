    ...
    for line in pdf_lines:
        ...
        if re.match(r'^\d{8}\b', line_str):
            ...
            if len(parts) >= 3:
                ...
                if found_dbk and not found_dbk.upper().endswith("B"):
    ...
    for item_idx in range(max_rows):
        ...
        for field_name, r_info in item_rules.items():
            ...
            if "smart" in rule_type_raw.lower():
                ...
                ws[cell_ref] = "PCS" if ("PCS" in desc or "PC" in desc) else (rule_val if rule_val else "SET")
            elif "pdf" in rule_type_raw.lower() or col_letter == "S":
                r_val_lower = rule_val.lower().strip()
                f_name_lower = field_name.lower().strip()
                raw_val = ""
                
                if "igst %" in r_val_lower or "igst rate" in f_name_lower: raw_val = nums[5] if len(nums) > 5 else ""
                elif "igst amt" in r_val_lower or "igst amount" in f_name_lower: raw_val = nums[6] if len(nums) > 6 else ""
                elif "hs" in r_val_lower or "ritc" in f_name_lower or "hs code" in r_val_lower: raw_val = item.get("hs_code", "")
                elif "description" in r_val_lower or "description" in f_name_lower: raw_val = item.get("description_text", "")
                elif "dbk" in r_val_lower or "drawback" in f_name_lower or col_letter == "S": raw_val = item.get("dbk_found", "") 
                elif "weight" in r_val_lower or "net wt" in f_name_lower: raw_val = nums[0] if len(nums) > 0 else ""
                elif "qty" in r_val_lower or "quantity" in f_name_lower: raw_val = nums[1] if len(nums) > 1 else ""
                elif "rate" in r_val_lower: raw_val = nums[2] if len(nums) > 2 else ""
                elif "amount usd" in r_val_lower or "goods value" in f_name_lower or "amount" in r_val_lower: raw_val = nums[3] if len(nums) > 3 else ""
                elif "taxable" in r_val_lower: raw_val = nums[4] if len(nums) > 4 else ""

                if "=" in rule_val:
                    raw_val = apply_value_replacement(raw_val, rule_val)

                try:
                    if col_letter == "S": ws[cell_ref] = raw_val 
                    else: ws[cell_ref] = float(str(raw_val).replace(",", ""))
                except:
                    ws[cell_ref] = raw_val
