...
def extract_item_table_rows(pdf_lines):
    ...
    for line in pdf_lines:
        line_str = line.strip()
        # Look for 8-digit HS Code at the start of line
        if re.match(r'^\d{8}\b', line_str):
            ...
            if len(parts) >= 3:
                ...
                # Dynamic PDF Row extraction parsing
                if len(parts) >= 8:
...
def map_items_to_excel_dynamic(ws, parsed_items, item_rules, inv_sr_no=1, start_overall_sr=1, start_excel_row=2, default_invoice_no="", default_invoice_date=""):
    ...
    # 🎯 Date Clean Up (Ensure real date DD/MM/YYYY is written, not field label)
    clean_date = default_invoice_date
    ...
    for item_idx, item in enumerate(parsed_items):
        ...
        # Hardcoded Fixed Columns for Output Standard Format
        ws[f"G{curr_row}"] = overall_sr                   # Overall SR NO
        ws[f"H{curr_row}"] = inv_sr_no                    # Inv. Sr. No.
        ws[f"I{curr_row}"] = item_sr_no                   # Item Sr. No.
        ws[f"J{curr_row}"] = default_invoice_no           # Invoice No
        ws[f"K{curr_row}"] = clean_date                   # Invoice Date
        
        # Process Rules Set in Section 4
        for field_name, r_info in item_rules.items():
            ...
            if rule_type == "Constant Text":
                ...
                # Smart Detection for Unit (SET / PCS)
                ...
                if "hs code" in r_val_lower or "ritc" in r_val_lower:
                    ws[cell_ref] = item.get("hs_code", "")
                elif "description" in r_val_lower or "product" in r_val_lower:
                    ws[cell_ref] = item.get("description", "")
                elif "qty" in r_val_lower or "quantity" in r_val_lower:
                ...
                elif "amount usd" in r_val_lower or "goods value" in r_val_lower or "amount" in r_val_lower:
                ...
                elif "igst %" in r_val_lower or "igst rate" in r_val_lower:
                    val = item.get("igst_rate", "")
                    try: ws[cell_ref] = float(val.replace(",", ""))
                    except: ws[cell_ref] = val
                elif "igst amt" in r_val_lower or "igst amount" in r_val_lower:
